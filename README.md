...
3. Click the gym → **JOIN**. Your sprite walks over while the agent feed closes the loop: DMs Maya → she replies → Google Calendar hold → Luma auto-RSVP.
4. Flip **GHOST** — you go translucent on the map, alerts stay on, session doesn't train the preference graph.
5. ~35s in: a **serendipity event** pops with a 60s countdown ("3 people free near the fountain"). Accept it or watch it vanish.
